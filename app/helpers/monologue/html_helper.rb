...
module Monologue
  module HtmlHelper
    ...
    def activable_li_tag(*url, &block)
      content_tag :li,
                  capture(&block),
                  class: ('active' if url.any? { |u| current_page?(u) })
    end

    # +li+ tag with the 'active' class added if
    # the url is the current one with a link inside it pointing to that url.
    #
    ...
    def sidebar_section_for(title, section_class, &block)
      content_tag(:section, class: "widget widget-#{section_class}") do
