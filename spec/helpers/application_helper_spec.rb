...
describe Monologue::ApplicationHelper do
  describe 'creating the url for a given tag' do
    ...
    it 'should return a well formed url for the tag' do
      helper.tag_url(@tag).should eq(
        "#{Monologue::Engine.routes.url_helpers.root_path}tags/my_tag"
      )
    end
  end
  ...
  describe 'resolving the absolute image url' do
    it 'should create the fully qualified url for a relative url' do
      ...
      helper.absolute_image_url('/image.png').should eq(
  ...
  describe 'creating the sidebar section for a title and a block' do
    before(:each) do
      ...
      @result = helper.sidebar_section_for('my title', 'test') do
        @content
      end
    end
  ...
  describe 'social icons+links' do
    context 'render if enabled' do
      it 'generate rss <link> tag for <head>' do
        helper.rss_head_link.should have_xpath(
          '//link[' \
          " @href='http://test.host/feed' and " \
          " @rel='alternate' and " \
          " @title='RSS' and " \
      ...
      it 'generate rss icon' do
        helper.rss_icon.should have_xpath(
          '//a[' \
          " @href='http://test.host/feed' and " \
          " @class='social' and " \
          " @target='_blank'" \
        ...
        helper.rss_icon.should have_xpath("//i[@class='foundicon-rss']")
      end

      it 'generate github' do
        helper.github_icon.should have_xpath(
          '//a[' \
          "@href='http://github.com/#{Monologue::Config.github_username}' and "\
          "@class='social' and " \
          "@target='_blank'" \
          ']'
        )
      ...
      it 'generate twitter' do
        helper.twitter_icon.should have_xpath(
          '//a[' \
        ...
        helper.twitter_icon.should have_xpath("//i[@class='foundicon-twitter']")
      end

      it 'generate linkedin' do
        helper.linkedin_icon.should have_xpath(
          '//a[' \
          "@href='#{Monologue::Config.linkedin_url}' and " \
          "@class='social' and " \
          "@target='_blank' " \
          ']'
        )
        ...
        helper.linkedin_icon.should have_xpath(
          "//i[@class='foundicon-linkedin']"
        )
      end

      it 'generate googleplus' do
        helper.googleplus_icon.should have_xpath(
          '//a[' \
          "@href='#{Monologue::Config.google_plus_account_url}' and " \
          "@class='social' and " \
          "@target='_blank'" \
          ']'
        )
      ...
      it 'generate facebook' do
        helper.facebook_icon.should have_xpath(
          '//a[' \
          "@href='#{Monologue::Config.facebook_url}' and " \
          "@class='social' and " \
          "@target='_blank'" \
          ']'
        )
  ...
  describe 'calculating the label size for a tag' do
    ...
    it 'should return number of label sizes for a value equal to max' do
      tag = mock_model(Monologue::Tag, posts_count: 7)
      helper.size_for_tag(tag, 3, 7).should eq 5
    end

    it 'should return a logarithmic scaling between min and ' \
       'max for a value strictly in the given interval' do
      tag = mock_model(Monologue::Tag, posts_count: 6)
